...
if EXPERIMENT not in ["noLR", "sensspec", "baseline", "noCoT"]:
...
if (EXPERIMENT == "sensspec"):
    ...
    EST_LR = "original"
elif (EXPERIMENT == "noCoT"):
    # even though there is no CoT, we provide the one from the vignette
    EST_LR = "original"
...
pretest_noCoT_instructions = """Let’s break the problem into multiple steps, given the medical nature of the question. Think through your reasoning to arrive at your estimate of disease probability. However, **do not** include any reasoning in your answer. Your answer should **only** include the probability estimate."""

posttest_noCoT_instructions = """Let’s break the problem into multiple steps, given the medical nature of the question. Think through your reasoning to arrive at your estimate of disease probability. However, **do not** include any reasoning in your answer. Your answer should **only** include the probability estimate."""
...
with_message_history = RunnableWithMessageHistory(
    ...
    history_factory_config=[
        ...
        ConfigurableFieldSpec(
            ...
            description="Unique identifier for the conversation with the trial #.",
            default="",
            is_shared=True,
        ),
    ],    
...
# pretest
if EXPERIMENT == "noCoT":
    pretest_reasoning = pretest_noCoT_instructions
else:
    pretest_reasoning = pretest_reasoning_instructions
# posttest 
if EXPERIMENT == "noLR":
    posttest_reasoning = posttest_reasoning_instructions
elif EXPERIMENT == "noCoT":
    posttest_reasoning = posttest_noCoT_instructions
else: 
    posttest_reasoning = posttest_withmath_reasoning_instructions
    
reasoning_instructions = {
    "pretest":pretest_reasoning,
    "posttest":posttest_reasoning,
}
...
for trialnum in range(TRIALS):
    ...
    for data_idx, case in data.iterrows():
        ...
        convo_history = get_session_history(str(vignetteid), f"{None}-{str(trialnum)}-pos")
        ...
        # Get all the data from the conversation
        convo_history = get_session_history(str(vignetteid), f"{None}-{str(trialnum)}-neg")
        if len(convo_history.messages) > 1:
            # if we have more than one message, we got a response back, otherwise we didn't             
...
print("Generating dataframe...", flush=True)
outputs_df = pd.DataFrame.from_records(outputs)
if TESTRUN:
    outputs_df.to_csv(f"/home/vs428/project/Uncertainty_data/smdm/test_results/{EXPERIMENT}-{ENGINE}-{TEMPERATURE}_results-smdm-test.csv", index=None)
else:
    outputs_df.to_csv(f"/home/vs428/project/Uncertainty_data/smdm/{EXPERIMENT}-{ENGINE}-{TEMPERATURE}_results-smdm-v1.csv", index=None)    
